...
### 多会话并行：必须用 git worktree，禁止共用主目录切换分支
...
- 若主目录 `D:\StepMark` 当前**已被另一个会话占用**（挂在非 master 分支、或正在改/编译），**当前会话必须用 worktree 在主目录外开独立工作树**，不能在主目录里 checkout：
  ```bash
  git worktree add ../StepMark-<功能名> -b <type>/<功能名>
  cd ../StepMark-<功能名>
  ```
  每个 worktree 有独立的工作树和 HEAD，多个会话各在自己的目录里 checkout/编译/合并，互不干扰。
- 若主目录空闲（挂在 master 且 clean），单会话可直接在主目录按下面的流程 checkout 分支，不强制建 worktree（向后兼容）。

### 流程（按顺序执行）

1. **确认 master 干净**：`git checkout master && git status`（主目录）或在 worktree 内直接开始。工作树必须 clean；若有残留改动，先提交或让用户确认后再继续。**禁止在脏的工作树上直接拉分支**，否则脏改动会污染新分支。
2. **拉功能分支**：`git checkout -b feat/<功能名>`（修复用 `fix/`，重构用 `refactor/`，文档用 `docs/`）。多会话则改用 worktree（见上）。分支名用英文、能体现功能。
3. **小步提交**：改一次 commit 一次，一个 commit 只表达一件事。不要等全部做完才一次性提交。提交信息用英文，遵循 `type(scope): summary` 格式（如 `feat(smart): ...`、`refactor(numbering): ...`）。
4. **合并前验证**：在功能分支/worktree 上跑 `npm test` 和 `npm run build`，确认通过后再合并。涉及 Konva/React 渲染的改动还要做手动 smoke test。**编译产物（exe）前必须确认当前分支和工作树代码就是本次改动**——编译前后各执行一次 `git branch --show-current` 和核对关键文件内容，防止多会话竞态把代码切走。
5. **等用户确认后再合并**：改完并自测后，把分支名、改动摘要、待验证点告诉用户。**只有用户明确表示验证通过，才执行合并**。禁止自测通过就自行合并。涉及渲染的改动以用户的手动 smoke test 为准。
6. **合并回主线并清理**：`git checkout master && git merge feat/<功能名>`（在主目录或任意 worktree 执行），合并后删除功能分支 `git branch -d feat/<功能名>`。若用了 worktree，还要 `git worktree remove <worktree目录>` 清理独立工作目录（`git merge` 和 `git branch -d` 都不会自动删 worktree 文件夹；worktree 未 remove 时 `git branch -d` 会因占用而失败）。除非用户明确要求合到其它分线，否则一律合回 master。
